...
/***        Definitions       ***/
struct clump {
    int type;          /* See denki.h for the list of types */

    int w,h;           /* Width/Height of the smallest clump's bounding rectangle */
    short int *shape;  /* Matrix that describes the clump */

    int x,y;           /* top left coordinates of the clump on the board */

    /* Clump's are gathered in a double linked list */
    struct clump *next; 
    struct clump *prev;
...
// FIXME: generalize level loading
#define LEVELS 6
/***       /Definitions       ***/
/***    Global variables      ***/
#include "variables.h"

static struct clump *whom[18][18];

static struct clump *clump_list=NULL;
static int clumps[9];
/***   Global variables       ***/

/*
 * Text debug output to check board layout
 */
...
/*
 * Text debug about clump content
 */
...
static void _free_clump_list( struct clump *l )
{
    if( l )
    {
...
static void free_clump_list( void )
{
    _free_clump_list( clump_list );
}

// TODO: retrouver à quoi ça sert
static void get_clump(int color, int i, int j, int rect[4], 
...
/*
 * Setup initial clump list
 */
static void build_clump_list( void )
{
    int i,j,k,l;
    unsigned char color;
